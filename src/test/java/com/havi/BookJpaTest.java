...
@DataJpaTest
public final class BookJpaTest {
    private final static String BOOK_TEST_TITLE = "Spring Boot Test Book";
    ...
    public void Book_저장하기_테스트() {
        Book book = Book.builder().title(BOOK_TEST_TITLE).
    ...
    public void BookList_저장하고_검색_테스트(){
        Book book1 = Book.builder().title(BOOK_TEST_TITLE+"1").
                publishedAt(LocalDateTime.now()).build();
        testEntityManager.persist(book1);

        Book book2 = Book.builder().title(BOOK_TEST_TITLE+"2").
                publishedAt(LocalDateTime.now()).build();
        testEntityManager.persist(book2);

        Book book3 = Book.builder().title(BOOK_TEST_TITLE+"3").
                publishedAt(LocalDateTime.now()).build();
